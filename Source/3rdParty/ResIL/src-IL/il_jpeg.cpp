...
{
	/*Header[0] = gIO.getc(gIO.io);
	Header[1] = gIO.getc(gIO.io);
	return;*/
	return (ILint) io->read(io, Header, 1, 2);
}
...
// Overrides libjpeg's stupid error/warning iors. =P
void ExitErrorio (struct jpeg_common_struct *JpegInfo)
{
	il2SetError(IL_LIB_JPEG_ERROR);
...
fill_input_buffer (j_decompress_ptr cinfo)
{
	iread_ptr src = (iread_ptr) cinfo->src;
	SIO * io = &src->image->io;
	ILint nbytes = (ILint) io->read(io, src->buffer, 1, INPUT_BUF_SIZE);
...
GLOBAL(void)
devil_jpeg_read_init (ILimage* image, j_decompress_ptr cinfo)
{
	iread_ptr src;
...
{
	iwrite_ptr dest = (iwrite_ptr)cinfo->dest;
	SIO* io = &dest->image->io;
	io->write(dest->buffer, 1, OUTPUT_BUF_SIZE, io);
	dest->pub.next_output_byte = dest->buffer;
	dest->pub.free_in_buffer = OUTPUT_BUF_SIZE;
	return IL_TRUE;
}
...
term_destination (j_compress_ptr cinfo)
{
	iwrite_ptr dest = (iwrite_ptr)cinfo->dest;
	SIO* io = &dest->image->io;
	io->write(dest->buffer, 1, OUTPUT_BUF_SIZE - (ILuint)dest->pub.free_in_buffer, io);
	return;
}


GLOBAL(void)
devil_jpeg_write_init(ILimage* image, j_compress_ptr cinfo)
{
	iwrite_ptr dest;
...
ILboolean iSaveJpegInternal(ILimage* image)
{
	...
	devil_jpeg_write_init(image, &JpegInfo);
...
ILboolean ilLoadFromJpegStruct(ILimage* image, void *_JpegInfo)
{
#ifndef IL_NO_JPG
#ifndef IL_USE_IJL
	// sam. void (*errorior)(j_common_ptr);
	ILubyte	*TempPtr[1];
	ILuint	Returned;
	...
	// sam. errorior = JpegInfo->err->error_exit;
	// sam. JpegInfo->err->error_exit = ExitErrorio;
	jpeg_start_decompress((j_decompress_ptr)JpegInfo);

	if (!il2TexImage(image, JpegInfo->output_width, JpegInfo->output_height, 1, (ILubyte)JpegInfo->output_components, 0, IL_UNSIGNED_BYTE, NULL)) {
		return IL_FALSE;
	}
	image->Origin = IL_ORIGIN_UPPER_LEFT;
	...
	// sam. JpegInfo->err->error_exit = errorior;
...
ILboolean ilSaveFromJpegStruct(ILimage* image, void *_JpegInfo)
{
	...
	//errorior = JpegInfo->err->error_exit;
	JpegInfo->err->error_exit = ExitErrorio;
