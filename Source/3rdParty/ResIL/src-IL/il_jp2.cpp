...
ILboolean iIsValidJp2(SIO* io)
{
	...
	io->seek(io, 4, IL_SEEK_CUR);  // Skip the 4 bytes that tell the size of the signature box.
	if (io->read(io, Signature, 1, 4) != 4) {
		io->seek(io, -4, IL_SEEK_CUR);
		return IL_FALSE;  // File read error
	}

	io->seek(io, -8, IL_SEEK_CUR);  // Restore to previous state
...
ILboolean iLoadJp2Internal(ILimage* image, jas_stream_t	*Stream)
{
	...
	switch (jas_image_numcmpts(Jp2Image))
	{
		...
		case 1:  // Assuming this is luminance data.
			if (image == NULL) {
				il2TexImage(image, jas_image_width(Jp2Image), jas_image_height(Jp2Image), 1, 1, IL_LUMINANCE, IL_UNSIGNED_BYTE, NULL);
				TempImage = image;
			}
			else {
				ifree(image->Data);  // @TODO: Not really the most efficient way to do this...
				il2InitImage(image, jas_image_width(Jp2Image), jas_image_height(Jp2Image), 1, 1, IL_LUMINANCE, IL_UNSIGNED_BYTE, NULL);
				TempImage = image;
			}
			break;
		...
		case 2:  // Assuming this is luminance-alpha data.
			if (image == NULL) {
				il2TexImage(image, jas_image_width(Jp2Image), jas_image_height(Jp2Image), 1, 2, IL_LUMINANCE_ALPHA, IL_UNSIGNED_BYTE, NULL);
				TempImage = image;
			}
			else {
				ifree(image->Data);  // @TODO: Not really the most efficient way to do this...
				il2InitImage(image, jas_image_width(Jp2Image), jas_image_height(Jp2Image), 1, 2, IL_LUMINANCE_ALPHA, IL_UNSIGNED_BYTE, NULL);
				TempImage = image;
			}
			break;
		...
		case 3:
			if (image == NULL) {
				il2TexImage(image, jas_image_width(Jp2Image), jas_image_height(Jp2Image), 1, 3, IL_RGB, IL_UNSIGNED_BYTE, NULL);
				TempImage = image;
			}
			else {
				ifree(image->Data);  // @TODO: Not really the most efficient way to do this...
				il2InitImage(image, jas_image_width(Jp2Image), jas_image_height(Jp2Image), 1, 3, IL_RGB, IL_UNSIGNED_BYTE, NULL);
				TempImage = image;
			}
			break;
		case 4:
			if (image == NULL) {
				il2TexImage(image, jas_image_width(Jp2Image), jas_image_height(Jp2Image), 1, 4, IL_RGBA, IL_UNSIGNED_BYTE, NULL);
				TempImage = image;
			}
			else {
				ifree(image->Data);  // @TODO: Not really the most efficient way to do this...
				il2InitImage(image, jas_image_width(Jp2Image), jas_image_height(Jp2Image), 1, 4, IL_RGBA, IL_UNSIGNED_BYTE, NULL);
				TempImage = image;
			}
			break;
		default:
...
static long iJp2_file_seek(jas_stream_obj_t *obj, long offset, int origin)
{
	...
	{
		case SEEK_SET:
			return (long) iCurImage->io.seek(&iCurImage->io, offset, IL_SEEK_SET);
		case SEEK_CUR:
			return (long) iCurImage->io.seek(&iCurImage->io, offset, IL_SEEK_CUR);
		case SEEK_END:
			return (long) iCurImage->io.seek(&iCurImage->io, offset, IL_SEEK_END);
	}
	return 0;  // Failed
}
