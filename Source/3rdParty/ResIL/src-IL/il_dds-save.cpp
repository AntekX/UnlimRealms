...
ILboolean WriteHeader(ILimage *curImage, ILenum DXTCFormat, ILuint CubeFlags)
{
	...
	*/
	...
	SaveLittleUInt(&curImage->io,il2GetInteger(IL_NUM_MIPMAPS) + 1);  // MipMapCount
	...
	if (il2GetInteger(IL_NUM_MIPMAPS) > 0)
...
ILubyte* ILAPIENTRY il2GetDXTCData(ILimage* curImage)
{
	if (curImage == NULL) {
	...
	return curImage->DxtcData;
}

ILAPI ILubyte* ILAPIENTRY il2GetMipDXTCData(ILimage* curImage, ILuint level)
{
	if (curImage == NULL) {
		il2SetError(IL_ILLEGAL_OPERATION);
		return NULL;
	...
	ILimage *mipImage = curImage->Mipmaps;
	for (ILuint i = 0; i < level; ++i)
	{
		mipImage = mipImage->Mipmaps;
	}

	return il2GetDXTCData(mipImage);
}
ILAPI ILubyte* ILAPIENTRY il2GetDXTCData(ILimage* curImage, ILuint level)
{
	if (curImage == NULL) {
		il2SetError(IL_ILLEGAL_OPERATION);
		return NULL;
