...
//! Returns a pointer to the current image's DXTC data.
ILAPI ILubyte*  ILAPIENTRY ilGetDXTCData(void)
{
	return il2GetDXTCData(iCurImage);
}

ILAPI ILubyte*  ILAPIENTRY ilGetMipData(ILuint level)
{
	return il2GetMipData(iCurImage, level);
}

ILAPI ILubyte*  ILAPIENTRY ilGetMipDXTCData(ILuint level)
{
	return il2GetMipDXTCData(iCurImage, level);
}
