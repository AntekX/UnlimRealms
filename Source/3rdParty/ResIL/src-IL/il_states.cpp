...
ILconst_string _ilVendor		= IL_TEXT("");
ILconst_string _ilVersion		= IL_TEXT("Resilient Image Library (ResIL) 1.8.3");
...
const char* _ilLoadExt	= "" IL_BLP_EXT IL_BMP_EXT IL_CUT_EXT IL_DCX_EXT IL_DDS_EXT
...
const char* _ilSaveExt	= "" IL_BMP_EXT IL_CHEAD_EXT IL_DDS_EXT IL_EXR_EXT
...
void ilDefaultStates()
{
	...
	ilStates[ilCurrentPos].ilDecompressDxtcData = IL_TRUE;
...
ILconst_string ILAPIENTRY il2GetString(ILenum StringName)
{
	switch (StringName)
	{
		case IL_VENDOR:
	...
	switch (Mode) {
		...
		case IL_INTERLACE_MODE:
			*Param = ilStates[ilCurrentPos].ilInterlace;
			break;
		case IL_DECOMPRESS_DXTC:
			*Param = ilStates[ilCurrentPos].ilDecompressDxtcData;
			break;
...
ILAPI void ILAPIENTRY il2SetInteger(ILenum Mode, ILint Param)
{
	switch (Mode)
	{
		...
		case IL_DECOMPRESS_DXTC:
			if (Param == IL_FALSE || Param == IL_TRUE) {
				ilStates[ilCurrentPos].ilDecompressDxtcData = Param;
				return;
			}
			break;
		case IL_KEEP_DXTC_DATA:
	...
	il2SetError(IL_INVALID_PARAM);  // Parameter not in valid bounds.
	return;
}
