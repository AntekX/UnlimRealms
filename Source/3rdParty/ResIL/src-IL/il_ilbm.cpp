...
// file turns out to be an ILBM. I think the best solution would be to
...
static ILboolean load_ilbm(ILimage* image)
{
    ...
    if ( !image->io.read(&image->io, &size, 4, 1 ) )
    {
    ...
    while ( memcmp( id, "BODY", 4 ) != 0 )
    {
        ...
        {
            if ( !image->io.read(&image->io, &bmhd, sizeof( BMHD ), 1 ) )
            {
        ...
        {
            if ( !image->io.read(&image->io, &colormap, size, 1 ) )
            {
        ...
        {
            Uint32 viewmodes;
            if ( !image->io.read(&image->io, &viewmodes, sizeof(viewmodes), 1 ) )
            {
        ...
        if ( memcmp( id, "BODY", 4 ) )
        {
            ...
            if ( size ) image->io.seek(&image->io, size, IL_SEEK_CUR );
        }
    }
    ...
    if( !il2TexImage(image, width, bmhd.h, 1, (format==IL_COLOUR_INDEX)?1:3, format, IL_UNSIGNED_BYTE, NULL))
        goto done;
    ...
    if ( nbcolors>0 && flagHAM==0 )
    {
        ...
        if ( !pbm )
            il2RegisterPal(image, scratch_pal, 3*nbrcolorsfinal, IL_PAL_RGB24);
    ...
    for ( h=0; h < bmhd.h; h++ )
    {
        ...
        for ( plane=0; plane < nbplanes; plane++ )
        {
            ...
            if ( bmhd.tcomp == 1 )      /* Datas are compressed */
            {
                do
                {
                    if ( !image->io.read(&image->io, &count, 1, 1 ) )
                    {
                    ...
                    if ( count & 0x80 )
                    {
                        ...
                        if ( ( count > remainingbytes ) || !image->io.read(&image->io, &color, 1, 1 ) )
                        {
                    ...
                    else
                    {
                        ...
                        if ( ( count > remainingbytes ) || !image->io.read(&image->io, ptr, count, 1 ) )
                        {
                            if( count>remainingbytes)
                                il2SetError(IL_ILLEGAL_FILE_VALUE );
                           error="error reading BODY chunk";
            ...
            {
                if ( !image->io.read(&image->io, ptr, bytesperline, 1 ) )
                {
                    error="error reading BODY chunk";
                    goto done;
                }
