...
// Last modified by Björn in 2014
//
...
// Internal functions used to get the BLP header from the current file.
ILint iGetBlp2Head(SIO* io, BLP2HEAD *Header)
{
	return (ILint) io->read(io, Header, 1, sizeof(BLP2HEAD));
}


ILint iGetBlp1Head(SIO* io, BLP1HEAD *Header)
{
	return (ILint) io->read(io, Header, 1, sizeof(BLP1HEAD));
}
...
ILboolean iIsValidBlp2(SIO* io)
{
	BLP2HEAD Header;
	auto read = iGetBlp2Head(io, &Header);
	io->seek(io, -read, IL_SEEK_CUR);

	if (read < sizeof(BLP2HEAD))
		return IL_FALSE;
	else
...
ILboolean iCheckBlp1(BLP1HEAD *Header)
{
	...
	if (strncmp((const char*)Header->Sig, "BLP1", 4))
		return IL_FALSE;
...
ILboolean iIsValidBlp1(SIO* io)
{
	BLP1HEAD Header;
	auto read = iGetBlp1Head(io, &Header);
	io->seek(io, -read, IL_SEEK_CUR);

	if (read < sizeof(BLP1HEAD))
		return IL_FALSE;
	else
		return iCheckBlp1(&Header);
}
...
ILboolean iLoadBlp1(ILimage * Image)
{
	...
	switch (Header.Compression)
	{
		case BLP_TYPE_JPG:
			...
			return IL_FALSE;
#else
			JpegHeaderSize = GetLittleUInt(io);
			JpegHeader = (ILubyte*)ialloc(JpegHeaderSize);
			if (JpegHeader == NULL)
				return IL_FALSE;
			// Read the shared Jpeg header.
			if (io->read(io, JpegHeader, 1, JpegHeaderSize) != JpegHeaderSize) {
				ifree(JpegHeader);
				return IL_FALSE;
			}
			...
			//for (i = 0; i < 16; i++) {  // Possible maximum of 16 mipmaps
				...
				io->seek(io, Header.MipOffsets[i], IL_SEEK_SET);
				...
				if (io->read(io, JpegData + JpegHeaderSize, Header.MipLengths[i], 1) != 1)
					return IL_FALSE;

				// Send the data to the Jpeg loader
				//if (!ilLoadJpegL(JpegData, JpegHeaderSize + Header.MipLengths[i]))
					//return IL_FALSE;
				if (!il2LoadL(Image, IL_JPG, JpegData, JpegHeaderSize + Header.MipLengths[i]))
					return IL_FALSE;
		...
		case BLP_RAW:
			switch (Header.PictureType)
			{
				...
				case BLP_RAW_PLUS_ALPHA2:
					// Create the image.
					if (!il2TexImage(Image, Header.Width, Header.Height, 1, 4, IL_BGRA, IL_UNSIGNED_BYTE, NULL))
						return IL_FALSE;
					...
					if (io->read(io, Palette, 1, 1024) != 1024) {
						ifree(Palette);
						return IL_FALSE;
					}
					// Seek to the data and read it.
					io->seek(io, Header.MipOffsets[i], IL_SEEK_SET);						
					if (io->read(io, DataAndAlpha, Header.Width * Header.Height, 1) != 1) {
						ifree(DataAndAlpha);
						ifree(Palette);
						return IL_FALSE;
					}
	...
	switch (Header.Compression)
	{
			...
			for (Mip = 0; Mip < 16; Mip++) {  // Possible maximum of 16 mipmaps
				...
				switch (Header.AlphaBits)
				{
					case 0:
						...
						io->seek(io, Header.MipOffsets[Mip], IL_SEEK_SET);
						if (io->read(io, Image->Data, 1, Image->SizeOfData) != Image->SizeOfData)
							return IL_FALSE;
						break;
					...
					case 1:
						...
						// Seek to the data and read it.
						io->seek(io, Header.MipOffsets[Mip], IL_SEEK_SET);						
						if (io->read(io, DataAndAlpha, Image->Width * Image->Height, 1) != 1) {
							ifree(DataAndAlpha);
							ifree(Palette);
							return IL_FALSE;
						}
						...
						// Read in the alpha list.
						if (io->read(io, DataAndAlpha, AlphaSize, 1) != 1) {
							ifree(DataAndAlpha);
							ifree(Palette);
							return IL_FALSE;
						}
		...
		case BLP_DXTC:
			for (Mip = 0; Mip < 16; Mip++) {  // Possible maximum of 16 mipmaps
				...
				io->seek(io, Header.MipOffsets[Mip], IL_SEEK_SET);
				if (io->read(io, CompData, 1, Header.MipLengths[Mip]) != Header.MipLengths[Mip]) {
					ifree(CompData);
					return IL_FALSE;
				}
...
check_blp1:
	io->seek(io, -148, IL_SEEK_CUR);  // Go back the size of the BLP2 header, since we tried reading it.
